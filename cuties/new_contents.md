...
Let's first consider the case where the platform gives no favor to the new content, i.e., $\alpha = 1$. We can derive the optimal quality decision of the new content creator by solving the first order condition, which gives us the equation $q^\ast = c\sqrt{\bar{q}} - \bar{q}$.

In the symmetric equilibrium, the average quality $\bar{q}$ is equal to the optimal quality of the new content $q^\ast$. Substituting $\bar{q}$ with $q^\ast$ in the first order condition, we get $q^\ast = \frac{c^2}{4}$.

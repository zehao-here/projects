...
The recommendation is captured by two parameters, $m_A$ and $m_B$, where $m_A, m_B > 0$ and $m_A + m_B = \tfrac{1}{2}$. These can be understood as the endorsement power of the two groups. If both groups recommend agent 1, then agent 1 is chosen with probability $\frac{1}{2} + m_A + m_B = 1$, so agent 2 is never selected. If group A recommends agent 1 while group B recommends agent 2, then agent 1 is selected with probability $\tfrac{1}{2} + m_A - m_B$, and agent 2 is selected with probability $\tfrac{1}{2} - m_A + m_B$.
...
It turns out that the optimal strategy for group B is to support agent 2 if $b \geq (\frac{1}{2} - m_A + m_B)(\theta_H - \theta_L)\pi$. A sufficiently large personal benefit will make group B support its friend, harming the project's success.
